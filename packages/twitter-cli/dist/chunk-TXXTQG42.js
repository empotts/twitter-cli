...
var CACHE_DIR = join(homedir(), ".twitter-cli");
var CACHE_FILE = join(CACHE_DIR, "last_results.json");
...
async function run(argv = process.argv) {
  const program = new Command().name("twitter-ts").option("-c, --compact", "Compact output (minimal fields, LLM-friendly).").option("-v, --verbose", "Enable verbose logging.").showHelpAfterError();
...
function addAuthCommands(program) {
  ...
  auth.command("import <file>").description("Validate a cookie export and emit configuration for this CLI.").option("--write-env <file>", "Write TWITTER_COOKIE_STRING to an env file.").option("--json", "Output as JSON.").option("--yaml", "Output as YAML.").action(async (file, options) => {
    ...
    if (options.writeEnv) {
      await writeFile2(resolve(options.writeEnv), `${envLine}
`, "utf-8");
    }
    const data = {
      ...authStatusData(authConfig),
      env: envLine,
      writeEnv: options.writeEnv ? resolve(options.writeEnv) : void 0
    };
    const mode = resolveMode(options);
    const structured = emitSuccess(data, mode);
    if (structured) {
      ...
      const lines = [
        ...
        "",
        options.writeEnv ? `Wrote ${resolve(options.writeEnv)}` : `Run this in your shell:
${envLine}`
      ];
...
function addReadCommands(program) {
  ...
  program.command("article <tweetId>").option("--markdown", "Output article as Markdown.").option("-o, --output <file>", "Save article Markdown to file.").option("--json", "Output as JSON.").option("--yaml", "Output as YAML.").action(async (tweetId, options) => {
    ...
    if (options.markdown) {
      ...
      if (options.output) {
        await writeFile2(resolve(options.output), markdown, "utf-8");
      } else {
...
async function loadTweetsFromFile(filePath) {
  const raw = await readFile3(resolve(filePath), "utf-8");
  const parsed = JSON.parse(raw);
...
async function outputTweets(tweets, options) {
  ...
  if (options.outputFile) {
    await writeFile2(resolve(options.outputFile), JSON.stringify(tweetsToData(filtered), null, 2), "utf-8");
  }
...
//# sourceMappingURL=chunk-TXXTQG42.js.map